...
struct Tween {
	...
	/*
	type		: TType,
	int plays; // -1 = infini, 1 et plus = nombre d'ex�cutions (1 par d�faut)
	bool fl_pixel; // arrondi toutes les valeurs si TRUE (utile pour les anims pixelart)
...
class Tweenie {
	...
	public inline function create(Object parent, String varName, float to, ?tp:TType,flt duration_ms) {
		if( Reflect.hasField(parent,varName) )
			return create_(parent, varName, to, tp, duration_ms);
		else
			return create_(parent, untyped __unprotect__(varName), to, tp, duration_ms); // champ obfuscqu�
	}

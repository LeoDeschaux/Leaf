...
class ZipFilePack : FilePack
{
	...
	{
		String destination = scope String();
		
		if(destDir == "")
			System.IO.Path.GetDirectoryPath(zipPath, destination);
		else
	    ...
	    for (int i = 0; i < fileCount; i++)
	    {
			...
			/*
			Console.WriteLine(fileStat.mMethod);
			Console.WriteLine(fileStat.mCompSize);
			Console.WriteLine(fileStat.mUncompSize);
			Console.WriteLine("---");
			*/
	...
	public static void AddDirectoryToZip(ZipArchive* zip, String currentPath, String basePath)
	{
		...
		{
			var dirName = dir.GetFileName(.. scope .());
			//Log.Message(dirName);

			AddDirectoryToZip(zip, scope $"{currentPath}/{dirName}", basePath);
		}

	    for (var file in Directory.EnumerateFiles(currentPath))
	    {
			String fileName = file.GetFileName(.. scope .());
			String filePath = file.GetFilePath(.. scope .());
			//String relativePath = scope $"{currentPath}/{fileName}";
			String currentDir = Directory.GetCurrentDirectory(.. scope .());

			String relativePath = scope String(filePath.Substring(basePath.Length+1));
			String rawPath = scope $"{currentDir}/{filePath}";

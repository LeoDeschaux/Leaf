...
class Client : Leaf.Entity
{
	...
	private void SendMessageOld(String messageToSend)
	{
		if(socket.Connect(serverIp, port, out sock) case .Err)
		{
			Console.WriteLine("Failed to connect to server");
		...
		var recvRes = socket.Recv(buffer, 4096);
...
class Server : Leaf.Entity
{
	...
	private void ReceiveFromClient()
	{
		...
		if (socket.AcceptFrom(listener) case .Err(let err))
		{
			return;
		}
		...
		var res = socket.Recv(buffer, 4096);
